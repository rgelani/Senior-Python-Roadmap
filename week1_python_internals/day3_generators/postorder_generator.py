...
def postorder_traversal_generator(root):
    """
    Performs an Post-order traversal of a binary tree using a generator.
    ...
    if root is not None:
        ...
        yield from postorder_traversal_generator(root.left) 
        
        # 2. Traverse Right Subtree Recursively using yield from
        yield from postorder_traversal_generator(root.right)

        # 3. Yield the Root Key (The actual value processing)
        yield root.key 
...
if __name__ == "__main__":
    ...
    print("Post-Order Traversal (Generator Output):")
    
    # We loop over the generator object to consume the values
    traversal_gen = postorder_traversal_generator(root)
    ...
    lazy_gen = postorder_traversal_generator(root)
